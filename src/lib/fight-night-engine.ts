...
export class FightNightEngine {
  ...
  private updateBodyParts(boxer: Boxer): void {
    ...
    switch (boxer.state) {
      ...
      case 'uppercut':
        ...
        boxer.torso.y = -70; // Slight crouch
        break;
        
      case 'blocking':
        // Arms up in guard position
        boxer.leftArm.x = -25;
        boxer.rightArm.x = 25;
        boxer.leftArm.y = -130;
      ...
      case 'hit':
        // Recoil animation
        boxer.head.x = Math.random() * 10 - 5;
        boxer.torso.x = Math.random() * 8 - 4;
        break;
      ...
      default:
        // Reset to neutral boxing stance
        boxer.head = { x: 0, y: -140 };
        boxer.torso = { x: 0, y: -80 };
        boxer.leftArm = { x: -30, y: -100 };
        boxer.rightArm = { x: 30, y: -100 };
  ...
  private endRound(): void {
    ...
    if (aliveBoxers.length === 1) {
      ...
      const healthiest = this.gameState.boxers.reduce((prev, current) => 
